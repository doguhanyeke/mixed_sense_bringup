...
def generate_launch_description():
    xrce_gps_bridge_1 = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory('px4_gps'), 'launch', 'px4_gps_xrce_attack_deadlock.launch.py')),
        launch_arguments={
            'px4_ns': "px4_1",
            'gz_world_name': "swarm_deadlock",
    ...
    orca_1 = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory('ros2-orca'), 'launch', 'ros2_orca_deadlock_launch.py')),
        launch_arguments={
            'drone_id': "1",
        }.items(),
    )
